...
def dates(request):
	...
	sql = "select DISTINCT(DATE(datetime)) from data ORDER BY DATE(datetime) DESC;"
	cursor.execute(sql)
	rows = cursor.fetchall()
...
def traces_georss(request, date):
	...
	init_DB()
	sql = "SELECT * FROM data WHERE DATE(datetime)='%s';" % (date)
	cursor.execute(sql)
	rows = cursor.fetchall()
	...
	for r in rows:
		...
		#georss += "<p> Temp: " + str(r[5]) + "C </p>"
		#georss += "<p> Altitude: " + str(r[3]) + " m. </p>"
		#georss += "<p> Speed: " + str(r[4]) + " Km/h </p>"	
...
def traces(request, date):
	...
	r_data['num_traces'] = int(get_simple_sql ("SELECT COUNT(*) FROM data WHERE DATE(datetime)='%s';" % (date)))
	r_data['max_temp'] = float(get_simple_sql ("SELECT MAX(temperature) FROM data WHERE DATE(datetime)='%s';" % (date)))
	r_data['min_temp'] = float(get_simple_sql ("SELECT MIN(temperature) FROM data WHERE DATE(datetime)='%s';" % (date)))
	r_data['max_speed'] = float(get_simple_sql ("SELECT MAX(speed) FROM data WHERE DATE(datetime)='%s';" % (date)))	
	r_data['max_altitude'] = float(get_simple_sql ("SELECT MAX(altitude) FROM data WHERE DATE(datetime)='%s';" % (date)))
